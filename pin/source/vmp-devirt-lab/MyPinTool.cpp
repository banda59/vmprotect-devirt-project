...
{
    // Avoid REG_* token ambiguity caused by pinos/pbits headers on some toolchains.
    // Compare via Pin's string representation of the full register name instead.
    const REG fr = REG_FullRegName(r);
    const std::string s = REG_StringShort(fr);

    // IA32
    if (s == "esi" || s == "ebx" || s == "edi" || s == "ecx")
        return true;

    // IA32E
    if (s == "rsi" || s == "rbx" || s == "rdi" || s == "rcx" || s == "r8" || s == "r9")
        return true;
...
static VOID RecordBCRead(THREADID tid, ADDRINT ip, ADDRINT addr, UINT32 size)
{
    ...
    if (!ts->pend_on || ts->pend_used)
        return;

    if (ts->seq < ts->pend_seq || (ts->seq - ts->pend_seq) > 8)
        return;

    if (ip < ts->pend_ip || (ip - ts->pend_ip) >= 32)
        return;
...
static VOID Instruction(INS ins, VOID *v)
{
    ...
    bool is_handler = (!g_handlers.empty() && (g_handlers.find(ip) != g_handlers.end()));
    if (is_handler)
    {
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)PrepareHandlerEntry,
                       IARG_THREAD_ID,
                       IARG_INST_PTR,
                       IARG_END);
    }
